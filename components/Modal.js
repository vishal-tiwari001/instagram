...
function Modal() {
  ...
  const uploadPost = async () => {
    if (loading) return;
    setLoading(true);
    // we are creating a post and add to firebase 'post' collection
    // get the POST ID for the newly created post
    // upload the image to firebase storage with post ID
    // get a download URL from fb storage and update the orginal post with image
          ...
          >
            ...
            >
              <div>
                ...
                px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-800 focus:outline-none outline-none
                focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:text-sm disabled:bg-gray-300
                disable:cursor-not-allowed disabled:hover:bg-gray-300"
                    onClick={uploadPost}
                  >
